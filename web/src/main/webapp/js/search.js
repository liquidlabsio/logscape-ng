...
class Search {
    ...
    bind() {
        ...
        $.Topic(Logscape.Search.Topics.setSearchFiles).subscribe(function(returnedFileUrls) {
            console.log("Got SearchFile URLS:" + returnedFileUrls)
            self.setFileUrls(returnedFileUrls);
        })
        $.Topic(Logscape.Search.Topics.setSearchFileResults).subscribe(function(fileResults) {
            console.log("Set Bucket Results histo & raw:" + fileResults)
            self.setSearchFileResults(fileResults[0], fileResults[1])
        })
        $.Topic(Logscape.Search.Topics.setFinalResult).subscribe(function(finalResult) {
    ...
    submitSearch(search) {
        this.search = search;
        $.Topic(Logscape.Search.Topics.submitSearch).publish(search);
    }
    setFileUrls(fileUrls) {
        ...
        this.searchFileUrls.forEach(function(fileUrl, index, arr){
            $.Topic(Logscape.Search.Topics.searchFile).publish(fileUrl, self.search)
        })
    ...
    setSearchFileResults(histoUrl, eventsUrl) {
        ...
        if (this.searchedFileUrls.size == searchFileUrls.size) {
            $.Topic(Logscape.Search.Topics.getFinalResult).publish(self.search, self.searchedFileUrls);
        }
    }
